  ...
  public void handle (@Nonnull final Callback [] aCallbacks) throws IOException, UnsupportedCallbackException
  {
    for (final Callback aCallback : aCallbacks)
    {
      if (aCallback instanceof AttachmentRequestCallback)
      {
        ...
        {
         if(aCallback == null) {
        	 LOGGER.error("Peique ->Esnull coñp");
         }
         LOGGER.error("Peique otro instance->"+  AttachmentRequestCallback.class.isInstance(aCallback));
         LOGGER.error("Peique location->"+AttachmentRequestCallback.class.getProtectionDomain().getCodeSource().getLocation());
         LOGGER.error("Peique location2->"+aCallback.getClass().getProtectionDomain().getCodeSource().getLocation());
         

         LOGGER.error("Peique names???->"+aCallback.getClass().getName()+"-"+AttachmentRequestCallback.class.getName());
         LOGGER.error("Peique canonical names???->"+aCallback.getClass().getCanonicalName()+"-"+AttachmentRequestCallback.class.getCanonicalName());
         LOGGER.error("Peique super names???->"+aCallback.getClass().getSuperclass().getName()+"-"+AttachmentRequestCallback.class.getSuperclass().getName());
