...
public class CustomRestExceptionHandler extends ResponseEntityExceptionHandler
{
                                                                          ...
                                                                          final WebRequest request)
  {
    ...
    return _buildBadRequestError (error);
  }
  ...
  protected ResponseEntity <byte []> handleConnectorException (final ConnectorException ex, final WebRequest request)
  {
    ...
    final HttpHeaders headers = new HttpHeaders ();
    headers.setContentType (new MediaType (MediaType.APPLICATION_XML, StandardCharsets.UTF_8));
    return new ResponseEntity <> (ConnectorExceptionHandler.getResponseErrorObjectBytes (ex), headers, ex.getStatus ());
  }
  ...
  protected ResponseEntity <byte []> handleExceptionUnknown (final Exception ex, final WebRequest request)
  {
    ...
    final HttpHeaders headers = new HttpHeaders ();
    headers.setContentType (new MediaType (MediaType.APPLICATION_XML, StandardCharsets.UTF_8));
    return new ResponseEntity <> (ConnectorExceptionHandler.getGenericResponseError (ex), headers, HttpStatus.INTERNAL_SERVER_ERROR);
  }
                                                                     ...
                                                                     final WebRequest request)
  {
    ...
    final String err = MessageUtils.format (CMessageKeys.ERROR_400_MIMETYPE, args.toArray ());
    return _buildBadRequestError (err);
  }
                                                                  ...
                                                                  final WebRequest request)
  {
    ...
    if (ex.getCause () instanceof UnmarshalException)
    {
      ...
      error = MessageUtils.format (CMessageKeys.ERROR_400_UNMARSHALLING, args);
    }
    else
    {
      error = MessageUtils.format (CMessageKeys.ERROR_400_ARGS_REQUIRED, null);
    }
    return _buildBadRequestError (error);
  }
