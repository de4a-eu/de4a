...
public class MessageExchangeManager {
     ...
     * @return void
     */
    public void processMessageExchange(final MessageExchangeWrapper messageWrapper) {
        ...
        if (meMessage.payloads().size() >= 2) {
            ...
            LOGGER.error("Incoming message seems to be ill-formatted - too few payloads. "
                    + "Trying first one.");
            payload = meMessage.payloads().getFirst();
        }
        Document docMsg = DOMUtils.newDocumentFromInputStream(payload.getData().getInputStream());
