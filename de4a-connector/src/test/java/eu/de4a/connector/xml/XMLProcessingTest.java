...
public class XMLProcessingTest {
    ...
    public void testRegRepWrapMessage() {
        InputStream isReq = this.getClass().getClassLoader().getResourceAsStream("xml/request-usi.xml");
        Document doc;
        try {
            doc = DOMUtils.byteToDocument(isReq.readAllBytes());        
        
            Element requestWrapper = null;
    ...
    public void testMarshaller() {
        String evidencia = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
    ...
    public void testUjiResponse() throws MessageException, ParserConfigurationException {
        ...
        Document docUji = DOMUtils.stringToDocument(xml);
        
        Element title = (Element) DOMUtils.getNodeFromXpath(
        ...
        Document docTitle = DOMUtils.newDocumentFromNode(title, "HigherEducationDiploma");
        System.out.println(DOMUtils.documentToString(docTitle));
        
        DE4AT41Marshaller <HigherEducationDiplomaType> marshaller = DE4AT41Marshaller.higherEducationDiploma();
        HigherEducationDiplomaType diplomaType1 = marshaller.read(title);
        HigherEducationDiplomaType diplomaType = marshaller.read(docTitle);
